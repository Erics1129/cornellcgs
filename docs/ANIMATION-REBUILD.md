...
- **eye v3**: A generated flat iris material (`iris-v3.png` / `iris-v3.webp`) supplies irregular slate-blue fibres and an amber collar. The shader supplies the asymmetric pearl almond, spherical lighting, pupil dilation, curved code reflection and moving lid shadow. The upper lid does most of each blink; gaze shifts settle at held targets instead of following a constant pendulum. There is no lifelike skin or veins. `eye-still-v3.webp` is an actual renderer export with the code visible for the no-WebGL fallback; reduced motion uses an open, centered live-rendered still. Earlier eye artwork remains unused.
