...
for(const [engine,type] of [['Chromium',chromium],['WebKit',webkit]]){
 ...
 for(const [device,w,h] of [['desktop',1440,900],['phone',390,844]]){
  ...
  try{
   ...
   const cityShot=()=>city.screenshot({style:'main,.cgs-nav,aside,.film-grain,footer{visibility:hidden!important}'});const a=await cityShot();await p.waitForTimeout(300);assert.ok(!a.equals(await cityShot()),'city travels automatically')
   await p.screenshot({path:`/tmp/cgs-final-${engine}-${device}-city.png`})
   for(const theme of ['sky','lilac']){await p.evaluate(theme=>{document.documentElement.dataset.theme=theme;window.dispatchEvent(new CustomEvent('cgs:theme',{detail:{theme}}))},theme);await p.waitForTimeout(1350);assert.equal(await p.locator('.code-city__mist').evaluate(e=>getComputedStyle(e).opacity),'1');await p.screenshot({path:`/tmp/cgs-final-${engine}-${device}-${theme}.png`})}
   ...
   await goto(.06);await p.screenshot({path:`/tmp/cgs-final-${engine}-${device}-globe.png`});assert.equal(await city.getAttribute('data-playback'),'covered')
   ...
   await goto(.12);await p.locator('.earth-intro-copy .scene-link').focus();await p.waitForTimeout(200);const before=await p.evaluate(()=>scrollY);await p.locator('.earth-intro-copy .scene-link').click();await p.waitForTimeout(600);assert.ok(p.url().includes('/world/'));assert.equal(await p.locator('.code-city').count(),0,'subpage has no city');await p.goBack();await p.waitForTimeout(2200);assert.ok(Math.abs(before-await p.evaluate(()=>scrollY))<3,'return retains exact position')
   ...
   const staticA=await cityShot();await p.waitForTimeout(200);assert.ok(staticA.equals(await cityShot()),'reduced city static')
